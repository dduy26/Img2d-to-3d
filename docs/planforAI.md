...
* **File chịu trách nhiệm:** `notebook/backend/engine_depth.py` & `notebook/backend/engine_dust3r.py`
* **Vấn đề cốt lõi:** Ước tính chiều sâu chính xác và đồng bộ hình học; loại bỏ các điểm rách biên (Edge-Bleeding) gây mạng nhện.
* **Giải pháp chuẩn NVIDIA 3D:**
  1. **Monocular & Multi-View Depth:** `Depth-Anything-V2-Small` dự đoán $D(u, v)$ trong 0.2s/ảnh với độ chi tiết cao.
  2. **Lọc viền bất liên tục (DA3 Edge Discontinuity Filter):**
     $$\nabla D(u, v) = \max\big(|\nabla D_x|, |\nabla D_y|\big)$$
     Triệt tiêu các pixel viền có $\nabla D > \tau \cdot D(u, v)$ trước khi đưa vào không gian 3D.
  3. **DUSt3R Global Alignment (Tùy chọn GPU):** Tối ưu hóa Bundle Adjustment trên các cặp ảnh để tính toán tọa độ thế giới khi không có camera chuẩn.

---

## 3️⃣ PHÂN HỆ 3: QUALITY GATE & FAIL-SAFE ENGINEER (P3 - ĐỨC)
* **File chịu trách nhiệm:** `notebook/backend/quality_gate.py`
* **Vấn đề cốt lõi:** Người dùng chụp thiếu góc (overlap $< 40\%$) hoặc chụp 2 ảnh trùng góc làm suy biến ma trận hình học.
* **Giải pháp chuẩn NVIDIA 3D:**
  ...
  3. **Lớp 3 - Fail-Safe Fallback (Thay thế TripoSR bằng Depth-Anything-V2 Surface Engine):**
     - Khi chỉ có 1 ảnh HOẶC khi bộ ảnh đa góc không đạt chuẩn $\to$ Kích hoạt `DepthReconstructionEngine` (~1.58s).
     - Loại bỏ hoàn toàn TripoSR (~1.7GB và C++ `torchmcubes`) để máy chủ không bao giờ bị treo.
